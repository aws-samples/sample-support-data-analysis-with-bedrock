...
MAKITA (Machine Augmented Key Infrastructure Technology Automation) implementation follows a bottom-up approach: project scaffolding, infrastructure-as-code, MCP server implementations, governance configuration, integration logic, monitoring, documentation, and testing. All MCP servers are implemented in Python using the Strands SDK. Infrastructure is defined in a single CloudFormation template.

## Tasks

- [x] 1. Project scaffolding and directory structure
  - [x] 1.1 Create the MAKITA project directory structure
    - Create top-level directories: `infrastructure/`, `mcp-servers/failover/`, `mcp-servers/precheck/`, `mcp-servers/postcheck/`, `mcp-servers/aws-support-stub/`, `mcp-servers/servicenow-stub/`, `event-logs/`, `tests/`
...
- [x] 2. CloudFormation template — single stack with all resources
  - [x] 2.1 Create the base CloudFormation template with PostgreSQL cluster resources
    - Create `infrastructure/makita-stack.yaml` with `AWSTemplateFormatVersion` and `Description`
    - Define `makita-pg-primary` RDS PostgreSQL instance in us-east-1
    - Define `makita-pg-replica` RDS Read Replica in us-west-2 with cross-region replication from primary
  ...
  - [x] 2.2 Add Parameter Store resources to the CloudFormation template
    ...
    - Define dashboard parameter: `/makita/dashboard/name`
  ...
  - [x] 2.4 Add AgentCore MCP Server registrations to the CloudFormation template
    - Define `makita-failover-mcp`, `makita-precheck-mcp`, `makita-postcheck-mcp` AgentCore McpServer resources
    - Define `makita-aws-support-stub`, `makita-servicenow-stub` AgentCore McpServer resources
    - Configure DevOps Agent connection to each MCP server
    - Apply mandatory tags (`auto-delete=no`, `Env=prod1`) to all AgentCore McpServer resources
    - _Requirements: 1.3, 5.1, 5.2, 5.3, 5.4, 18.2, 18.10, 19.2, 19.10, 24.1, 24.2, 24.9_

  - [x] 2.5 Add AgentCore Policies to the CloudFormation template
    - Define `makita-failover-policy` restricting failover operations to `makita-*` resources, us-east-1 → us-west-2 regions, and `makita-*` principals
    - Define `makita-precheck-policy` restricting pre-check operations to `makita-*` resources, us-east-1/us-west-2 regions, and `makita-*` principals
    - Define `makita-postcheck-policy` restricting post-check operations to `makita-*` resources, us-east-1/us-west-2 regions, and `makita-*` principals
    - Add `Env=prod1` tag-based constraint to all three policies: deny operations on resources that do not carry the `Env=prod1` tag, providing environment isolation in addition to the `makita-*` prefix constraint
    - Apply mandatory tags (`auto-delete=no`, `Env=prod1`) to all AgentCore Policy resources
  ...
  - [x] 2.6 Add AgentCore Identities to the CloudFormation template
    ...
    - Apply mandatory tags (`auto-delete=no`, `Env=prod1`) to all AgentCore Identity resources
    - _Requirements: 1.3, 9.4, 20.7, 20.8, 24.1, 24.2, 24.6_

  - [x] 2.7 Add Bedrock Guardrails to the CloudFormation template
    - Define `makita-failover-guardrail` with content filtering, topic restriction to DR operations, and prompt injection detection
    - Define `makita-precheck-guardrail` with content filtering, topic restriction to pre-check operations, and prompt injection detection
    - Define `makita-postcheck-guardrail` with content filtering, topic restriction to post-check operations, and prompt injection detection
  ...
  - [x] 2.8 Add CloudWatch Dashboard to the CloudFormation template
    - Define `makita-failover-dashboard` with widgets for Primary Region (us-east-1) and DR Region (us-west-2)
    - Include CPU utilization, DB connections, read/write IOPS, instance status widgets for both regions
    - Include cross-region replication lag and replication status widgets
    - Include health and availability status widgets
    - Note: `AWS::CloudWatch::Dashboard` does not support the Tags property; document as a tagging exception via inline comment in the template
    - _Requirements: 1.3, 16.1, 16.2, 16.3, 16.4, 16.5, 16.6, 24.8, 24.10_

  - [x] 2.9 Add CloudFormation Outputs and resource tagging
    - Define outputs for PrimaryEndpoint, ReplicaEndpoint, FailoverMcpServerArn, DashboardUrl
...
- [x] 4. Failover MCP Server implementation
  - [x] 4.1 Implement the Failover MCP Server core module
    - Create `mcp-servers/failover/server.py` using the Strands SDK
  ...
  - [x] 4.2 Implement FailoverResult and HealthCheckResult data models
    - Create `mcp-servers/failover/models.py` with `FailoverResult`, `HealthCheckResult`, and `FailoverState` dataclasses
...
- [x] 5. Pre-Check MCP Server implementation
  - [x] 5.1 Implement the Pre-Check MCP Server core module
    - Create `mcp-servers/precheck/server.py` using the Strands SDK
  ...
  - [x] 5.2 Implement VerificationResult data model for Pre-Check
    - Create `mcp-servers/precheck/models.py` with `VerificationResult` dataclass
    - _Requirements: 18.7_
...
- [x] 6. Post-Check MCP Server implementation
  - [x] 6.1 Implement the Post-Check MCP Server core module
    - Create `mcp-servers/postcheck/server.py` using the Strands SDK
  ...
  - [x] 6.2 Implement VerificationResult data model for Post-Check
    - Create `mcp-servers/postcheck/models.py` with `VerificationResult` dataclass
    - _Requirements: 19.7_
...
- [x] 14. README.md documentation
  - [x] 14.1 Create the project README.md
    ...
    - Include Mermaid architecture diagram showing all components: DevOps Agent, AgentCore, all MCP servers, stub servers, PostgreSQL cluster (us-east-1/us-west-2), Parameter Store, CloudWatch Dashboard, Bedrock Guardrails, AgentCore Policies/Identities
  ...
  - [x] 14.2 Create the standalone architectural diagram
    ...
    - Include CloudWatch Dashboard (`makita-failover-dashboard`)
    - Include MCP servers: Failover, Pre-Check, Post-Check
    - Include stub servers: AWS Support Stub, ServiceNow Stub
    - Include AgentCore governance: Policies, Identities, Bedrock Guardrails
    - Include DevOps Agent with connections to all five MCP/stub servers
...
- [x] 15. Comprehensive testing scripts
  ...
  - [x] 15.10 Create CloudWatch Dashboard tests
    - Write tests verifying dashboard creation with `makita-` prefix
    - Write tests verifying metrics from both us-east-1 and us-west-2 for PostgreSQL cluster
  ...
  - [x] 15.12 Create architectural diagram tests
    ...
    - Write tests verifying all required components are present in the diagram: PostgreSQL primary (us-east-1), PostgreSQL replica (us-west-2), replication relationship, CloudWatch Dashboard, Failover MCP Server, Pre-Check MCP Server, Post-Check MCP Server, AgentCore Policies, AgentCore Identities, Bedrock Guardrails, AWS Support Stub Server, ServiceNow Stub Server, DevOps Agent, Parameter Store
    - Write tests verifying DevOps Agent connections to all five MCP/stub servers are shown
    - _Requirements: 26.1, 26.2, 26.3, 26.4, 26.5, 26.6, 26.7, 26.8, 26.9, 26.10, 26.11_

- [x] 16. Final checkpoint — Ensure all tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 17. AgentCore Runtime and Gateway
  - [ ] 17.1 Package MCP server code for S3 deployment
    - Create a packaging script (`scripts/package-mcp.sh`) that zips the `mcp-servers/` directory
    - Upload the ZIP artifact to an S3 bucket (`makita-artifacts-{account_id}`) in us-east-1
    - Output the S3 bucket and key for use by the AgentCore stack
    - _Requirements: 30.1, 30.2, 30.3_

  - [ ] 17.2 Update AgentCore stack with Runtime and Endpoint
    - Add `create_agent_runtime` call to the Lambda: create `makita-mcp-runtime` with code configuration (S3 artifact, Python 3.11, MCP protocol, PUBLIC network)
    - Add `create_agent_runtime_endpoint` call: create `makita-mcp-runtime-endpoint` referencing the Runtime
    - Pass S3 bucket and key as Custom Resource properties
    - _Requirements: 27.1, 27.2, 27.3, 27.4, 27.5, 27.6, 27.7_

  - [ ] 17.3 Update AgentCore stack with Gateway and Gateway Targets
    - Add `create_gateway` call: create `makita-mcp-gateway` with MCP protocol, IAM auth, failover role
    - Add `create_gateway_target` calls for each MCP server: failover, precheck, postcheck, aws-support-stub, servicenow-stub
    - Each target references the corresponding MCP server endpoint in the Runtime
    - _Requirements: 28.1, 28.2, 28.3, 28.4, 28.5, 28.6_

  - [ ] 17.4 Update AgentCore stack to use Workload Identities
    - Replace `create_identity` calls with `create_workload_identity` for each MCP server
    - Map each Workload Identity to its corresponding IAM role
    - _Requirements: 29.1, 29.2, 29.3_

  - [ ] 17.5 Update deploy script for AgentCore with packaging
    - Update `deploy_agentcore` function to run `scripts/package-mcp.sh` before deploying the stack
    - Pass S3 bucket and key as parameters to the AgentCore stack
    - _Requirements: 30.1, 30.2, 30.3_

- [ ] 18. Checkpoint — Validate AgentCore Runtime and Gateway deployment
  - Ensure the AgentCore stack deploys successfully with Runtime, Endpoint, Gateway, Targets, Workload Identities, and Policies. Ask the user if questions arise.
...
- Infrastructure is a single CloudFormation stack — no additional stacks required
...
- AgentCore Policies enforce `Env=prod1` tag constraint in addition to `makita-*` prefix, region, and principal constraints
- Resources that do not support CloudFormation tagging (e.g., `AWS::CloudWatch::Dashboard`) are documented as exceptions via inline comments
