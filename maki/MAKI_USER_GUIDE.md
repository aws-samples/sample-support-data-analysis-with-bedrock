...
pip install -r maki/tools/requirements.txt
```
...
**Health Events Mode**: Out of the box, MAKI expects health events to be populated in the OpenSearch Serverless collection created during deployment. Use `maki/tools/get_health_events.py` as a reference for pulling health events from the AWS Health API. For comprehensive health event aggregation patterns, see [AWS blog](https://aws.amazon.com/blogs/mt/how-to-aggregate-and-visualize-aws-health-events-using-aws-organizations-and-amazon-elasticsearch-service/).
...
python maki/tools/purge_s3_data.py # optional, to clean out any previous runs
...
python maki/tools/generate_synth_cases.py --min-cases 10 --max-cases 15
# rather than running the above every time, you can pre-generate the cases with the above tool,
# and use maki/tools/copy_s3_data.py (see directions for this tool under its own heading) 
...
python maki/tools/opensearch_client.py --size 150
...
python maki/tools/flip_mode.py --show

# Toggle between modes (automatic flip)
python maki/tools/flip_mode.py
```
...
- **Modified By**: `maki/tools/flip_mode.py` script or manual SSM console updates
...
# Configure for small dataset processing
python maki/tools/opensearch_client.py --size 10
...
# Configure for large dataset processing
python maki/tools/opensearch_client.py --size 200
...
- **Command**: `python maki/tools/generate_synth_cases.py` (uses default of 1-2 cases per category)
- **Custom Volume**: `python maki/tools/generate_synth_cases.py --min-cases 1 --max-cases 5` (override defaults)
...
python maki/tools/get_health_events.py --load-to-opensearch

# Verify events are loaded
python maki/tools/get_health_events.py --count-only
```
...
```bash
python maki/tools/flip_mode.py --mode cases  # or --mode health
```
...
python maki/tools/get_health_events.py --start-time "2023-01-01T00:00:00Z"
```
...
MAKI includes a comprehensive test plan with 7 distinct test scenarios to validate functionality across different modes and processing types. The test plan is located in `maki/tools/test_plan.md` and can be executed using:

```bash
python maki/tools/execute_test_plan.py              # Run all tests
python maki/tools/execute_test_plan.py --test-case 2 # Run only Test 2
python maki/tools/execute_test_plan.py --test-plan custom_test_plan.md # Use custom test plan file
```
...
python maki/tools/opensearch_client.py --size 1
python maki/tools/runMaki.py
```
...
python maki/tools/flip_mode.py --mode health
python maki/tools/opensearch_client.py --endpoint
python maki/tools/opensearch_client.py --size 101
python maki/tools/runMaki.py
```
...
- Use `python maki/tools/copy_s3_data.py from-temp` to reuse pre-generated test cases for Test 3
...
The `maki/tools/` directory contains utility scripts for managing, testing, and operating MAKI. Each script serves a specific purpose in the MAKI workflow:
...
- **Usage**: `python maki/tools/runMaki.py`
...
- **Usage**: 
  - `python maki/tools/flip_mode.py` (show current mode)
  - `python maki/tools/flip_mode.py --mode cases` (set to cases mode)
  - `python maki/tools/flip_mode.py --mode health` (set to health mode)
...
- **Usage**: `python maki/tools/generate_synth_cases.py --min-cases 1 --max-cases 5`
...
- **Usage**: `python maki/tools/generate_synth_health_events.py --synth 100 --verbose`
...
- **Usage**: `python maki/tools/get_health_events.py --load-to-opensearch --start-time "2024-01-01T00:00:00Z"`
...
- **Usage**: `python maki/tools/purge_s3_data.py`
...
- **Usage**: `python maki/tools/copy_s3_data.py from-temp`
...
- **Usage**: 
  - `python maki/tools/opensearch_client.py --size 100` (set query size)
  - `python maki/tools/opensearch_client.py` (show current size)
...
- **Usage**: 
  - `python maki/tools/execute_test_plan.py` (run all tests)
  - `python maki/tools/execute_test_plan.py --test-case N` (run specific test 1-6)
  - `python maki/tools/execute_test_plan.py --test-plan custom_test_plan.md` (use custom test plan file)

### Script Dependencies and Requirements

All scripts share common dependencies defined in `maki/tools/requirements.txt`:
...
- Root directory: `python maki/tools/script_name.py`
